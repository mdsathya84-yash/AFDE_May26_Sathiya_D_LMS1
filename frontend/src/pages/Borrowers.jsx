...
import { createBorrower, updateBorrower, deleteBorrower } from '../api/api'
...
function validate(form) {
  const errors = {}
  if (!form.borrower_name.trim()) errors.borrower_name = 'Required'
...
function Modal({ title, form, setForm, errors, onSave, onClose, saving }) {
  const field = (key, label, required, type = 'text') => (
    <div style={ms.field}>
      ...
      <input style={ms.input} type={type} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} />
  ...
  return (
    <div style={ms.overlay}>
      <div style={ms.modal}>
        ...
        {field('borrower_name', 'Full Name', true)}
...
export default function Borrowers() {
  const { data: borrowers, loading, error, refetch } = useBorrowers()
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'add') {
        ...
        await updateBorrower(modal.borrower.borrower_id, form)
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.toolbar}>
        <h1 style={styles.heading}>Borrowers</h1>
        <button style={styles.addBtn} onClick={openAdd}>+ Add Borrower</button>
      </div>
      ...
      {loading ? <div style={styles.skeleton} /> : (
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {['Name', 'Email', 'Phone', 'Active Borrows', 'Actions'].map((h) => (
            ...
            <tbody>
              {borrowers.length === 0
                ? <tr><td colSpan={5} style={styles.empty}>No borrowers found.</td></tr>
                : borrowers.map((b) => (
                  <tr key={b.borrower_id}>
                    <td style={styles.td}>{b.borrower_name}</td>
                    <td style={styles.td}>{b.email}</td>
                    <td style={styles.td}>{b.phone || '—'}</td>
                    <td style={styles.td}>
                      {b.active_borrows > 0
                        ? <span style={styles.activeBadge}>{b.active_borrows}</span>
                        : <span style={styles.zeroBadge}>0</span>}
                    </td>
                    <td style={styles.td}>
                      <button style={styles.editBtn} onClick={() => openEdit(b)}>Edit</button>
                      <button style={styles.deleteBtn} onClick={() => handleDelete(b)}>Delete</button>
                    </td>
                  </tr>
                ))
              }
      ...
      {modal && (
        <Modal
          title={modal === 'add' ? 'Add Borrower' : 'Edit Borrower'}
          form={form}
          setForm={setForm}
          errors={formErrors}
          onSave={handleSave}
...
const styles = {
  ...
  activeBadge: { background: '#f59e0b', color: '#fff', fontSize: '0.75rem', padding: '2px 10px', borderRadius: 12 },
  zeroBadge: { background: '#e2e8f0', color: '#64748b', fontSize: '0.75rem', padding: '2px 10px', borderRadius: 12 },
...
const ms = {
  ...
  modal: { background: '#fff', borderRadius: 10, padding: '2rem', width: '100%', maxWidth: 440 },
